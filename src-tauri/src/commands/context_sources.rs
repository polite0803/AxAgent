...
pub async fn add_context_source(
    ...
    axagent_dao::repo::context_source::add_context_source(state.harness.db(), &input)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn remove_context_source(state: State<'_, AppState>, id: String) -> Result<(), String> {
    axagent_dao::repo::context_source::remove_context_source(state.harness.db(), &id)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn toggle_context_source(
    ...
    axagent_dao::repo::context_source::toggle_context_source(state.harness.db(), &id)
        .await
        .map_err(|e| e.to_string())
}
