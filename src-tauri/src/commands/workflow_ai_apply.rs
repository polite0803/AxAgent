...
fn apply_value_path(
    ...
    if segments.len() == 1 {
        if root_value.is_object() {
            root_value.as_object_mut().ok_or_else(|| "root_value is not an object".to_string())?.insert(segments[0].to_string(), value);
        } else {
    ...
    let (last, head) = segments.split_last().ok_or_else(|| "unreachable: segments should not be empty".to_string())?;
    ...
    cur.as_object_mut().ok_or_else(|| "cur is not an object".to_string())?.insert((*last).to_string(), value);
    Ok(())
}
