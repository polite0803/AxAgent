...
pub(crate) async fn do_compress(
        ...
        (&settings.compression_provider_id, &settings.compression_model_id)
    {
        match axagent_dao::repo::provider::get_provider(db, pid).await {
            Ok(p) => match p.keys.first() {
                Some(k) => {
                    let dk =
                        axagent_crypto::decrypt_key(&k.key_encrypted, master_key).map_err(|e| {
                            String::from(crate::commands::error::ErrorResponse::from_error(
                                e,
                                crate::commands::error::ErrorCategory::Unrecoverable,
                            ))
    ...
    let registry_key =
        axagent_harness::types::provider_model::provider_registry_key(&comp_provider.provider_type);
    let adapter = harness
        .provider_registry()
        .get(registry_key)
...
pub async fn compress_context(
    ...
    let collect_messages = |msgs: &[Message]| -> Result<Vec<ChatMessage>, String> {
        ...
        for m in msgs {
            ...
            out.push(chat_message_from_message(&file_store, m).map_err(|e| {
                String::from(crate::commands::error::ErrorResponse::from_error(
                    e,
                    crate::commands::error::ErrorCategory::Unrecoverable,
                ))
...
mod tests_conversation {
    ...
    async fn persist_attachments_registers_stored_files_for_files_page() {
        ...
        let state = crate::AppState {
            ...
            session_state_store: Arc::new(
                axagent_harness::test_support::MemorySessionStateStore::new(),
            ),
