...
use crate::commands::conversations::messages::streaming::build_working_memory_chat_message;
use crate::commands::conversations::messages::streaming::resolve_rag_ids;
use crate::commands::conversations::messages::streaming::dedup_rag_against_working_memory;
use crate::commands::conversations::messages::streaming::apply_rag_token_budget;
use crate::commands::conversations::messages::streaming::execute_tool_call;
use crate::commands::conversations::messages::streaming::generate_ai_title;
use crate::commands::conversations::messages::streaming::consume_stream;
use crate::commands::conversations::messages::streaming::build_memory_retrieval_tag;
use crate::commands::conversations::messages::streaming::build_rag_chat_message;
...
pub(super) fn spawn_stream_task(
    ...
    tokio::spawn(async move {
        tracing::info!("[spawn_stream_task] Task started for conversation={}", conversation_id);
...
response_format: None,
                ...
                let tc_json = serde_json::to_string(&tool_calls).inspect_err(|e| tracing::error!(%e, "serde_json 序列化失败")).ok();
                ...
                let intermediate_msg_id =
                    axagent_dao::repo::message::create_assistant_tool_call_message(
                        &db,
                        &conversation_id,
                        &content,
                        tc_json.as_deref(),
                    ...
                    .await
                    .unwrap_or_else(|_| axagent_kit::utils::gen_id());
                ...
                for tc in &tool_calls {
                    ...
                    let server_name = if tc.function.name == "web_search" {
                        ...
                        match axagent_dao::repo::mcp_server::find_server_for_tool(
                            &db,
                            &tc.function.name,
                            &mcp_server_ids,
                        )
                    ...
                    let exec = axagent_dao::repo::tool_execution::create_tool_execution(
                        &db,
                        &conversation_id,
                    ...
                    let _ = axagent_dao::repo::message::create_tool_result_message(
                        &db,
                        &conversation_id,
                        &tc.id,
                        &result_content,
            ...
            if let Err(e) = axagent_entities::messages::Entity::update(
                axagent_entities::messages::ActiveModel {
                    id: Set(assistant_message_id.clone()),
            ...
            if let Err(e) =
                axagent_dao::repo::conversation::increment_message_count(&db, &conversation_id)
                    .await
            {
...
pub async fn send_message(
    ...
    if let Some(ref pid) = conversation.agent_profile_id {
        if let Ok(Some(profile)) =
            axagent_entities::agent_profiles::Entity::find_by_id(pid.as_str())
    ...
    if let Ok(settings) = axagent_dao::repo::settings::get_settings(state.harness.db()).await {
        if !settings.language.is_empty() {
            let already_present = chat_messages.iter().any(|m| match &m.content {
                ChatContent::Text(t) => axagent_kit::utils::has_output_language_directive(t),
                _ => false,
            });
            if !already_present {
                let directive =
                    axagent_kit::utils::build_output_language_directive(&settings.language);
                chat_messages.push(ChatMessage {
                    role: if no_system_role {
                        "user".to_string()
                    } else {
        ...
        )
    {
        ...
        .await
        {
            ...
            let _ = axagent_dao::repo::message::create_message(
                state.harness.db(),
                &conversation_id,
                MessageRole::System,
    ...
    let tools: Option<Vec<ChatTool>> = if mcp_ids.is_empty() && !has_search_provider {
        ...
        if has_search_provider
            && super::super::super::is_builtin_tool_enabled("web_search", &disabled_tools_set, &group_enabled)
        {
