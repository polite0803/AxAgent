...
Write-Host "Total replacement chars in memory.rs: $($positions.Count)"
foreach ($pos in $positions) {
    ...
    for ($i = $start; $i -le $end; $i++) {
        if ($bytes[$i] -eq 0xEF -and $i + 2 -lt $bytes.Length -and $bytes[$i+1] -eq 0xBF -and $bytes[$i+2] -eq 0xBD) {
            ...
            Write-Host -NoNewline ('[U+{0:X4}]' -f ([System.Text.Encoding]::UTF8.GetString($bytes, $i, [Math]::Min(3, $bytes.Length - $i))[0]))
        }
    }
    Write-Host ''
}
