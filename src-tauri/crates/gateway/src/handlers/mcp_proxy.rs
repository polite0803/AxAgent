...
//!
//! 通过 harness 注入的 `McpServerStore` / `McpClientService` trait 访问 MCP，
//! 让 gateway 不直接依赖 `axagent-entities`、`axagent-mcp` 与 SeaORM。
...
pub async fn call_mcp_tool(
    ...
    match state.mcp_client.call_tool(&server, &body.tool_name, body.arguments).await {
        Ok(result) => (
            StatusCode::OK,
            Json(json!({
