...
impl Tool for FileWriteTool {
    ...
    fn execute_rollback(
        ...
        let action = record.payload["action"]
            .as_str()
            .ok_or_else(|| ToolError::invalid_input("execute_rollback: 缺少 action"))?;
        match action {
            "overwrite" => {
                let old_content = record.payload["old_content"].as_str().ok_or_else(|| {
                    ToolError::invalid_input("execute_rollback: 缺少 old_content")
                })?;
                std::fs::write(file_path, old_content)
                    .map_err(|e| ToolError::execution_failed(format!("回滚写入失败: {}", e)))?;
                Ok(ToolResult::success(format!("✅ 已回滚文件: {}", file_path)))
            },
            "create" => {
                std::fs::remove_file(file_path)
                    .map_err(|e| ToolError::execution_failed(format!("回滚删除失败: {}", e)))?;
                Ok(ToolResult::success(format!("✅ 已删除新文件: {}", file_path)))
            },
