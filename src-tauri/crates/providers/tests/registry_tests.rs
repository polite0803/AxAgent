...
fn test_registry_register_and_get() {
    ...
    registry.register("openai", Arc::new(axagent_providers::openai::OpenAIAdapter::new()));
    assert!(registry.get("openai").is_some());
}
...
fn test_registry_register_overwrites_existing() {
    let mut registry = ProviderRegistry::new();
    registry.register("test", Arc::new(axagent_providers::openai::OpenAIAdapter::new()));
    assert!(registry.get("test").is_some());

    // Register another adapter under the same key — should be fine
    registry.register("test", Arc::new(axagent_providers::anthropic::AnthropicAdapter::new()));
    assert!(registry.get("test").is_some());
}
