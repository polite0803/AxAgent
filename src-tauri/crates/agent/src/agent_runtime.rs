...
impl AgentRuntime {
    ...
    pub fn run(&mut self, input: &str) -> Result<AgentOutput, AgentRuntimeError> {
        ...
        // 将人格内容注入到 system prompt（根本修复）。
        //
        // 修复缺陷 8：原实现把 persona 拼到 user message 前，LLM 会把它当作用户内容。
        // 现在通过 ConversationRuntimeHost::set_system_directive 注入到真正的 system prompt，
        // 每次 LLM 调用时由 runtime 组装（见 runtime-core conversation.rs 的 system_prompt 组装）。
        if let Some(ref p) = personality_prompt {
            self.conversation_runtime.set_system_directive(p.clone());
        }
