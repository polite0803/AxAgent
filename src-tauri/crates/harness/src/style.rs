...
impl StyleExtractor for NoopStyleExtractor {
    async fn extract_from_code(&self, _: &[CodeSample]) -> Result<ExtractedCodePatterns, String> {
...
impl StyleApplier for NoopStyleApplier {
    async fn apply_style(&self, code: &str, _: &CodeStyleTemplate) -> Result<String, String> {
        Ok(code.to_string())
    }
    fn active_template(&self) -> Option<CodeStyleTemplate> {
        None
...
impl StyleVectorizer for NoopStyleVectorizer {
    async fn vectorize_code(&self, _: &CodeSample) -> Result<StyleVector, String> {
        Err("not configured".into())
    }
    async fn vectorize_message(&self, _: &MessageSample) -> Result<StyleVector, String> {
        Err("not configured".into())
    }
}
