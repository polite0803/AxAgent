...
//! 权限系统 — PermissionMode, PermissionPolicy 等类型定义。
//!
//! 不含 runtime-core 特有的 hook 触发逻辑（在 runtime-core 侧通过包装完成）。
...
// ── PermissionOverride / PermissionContext ────────────────────
...
// ── PermissionRequest / PermissionPromptDecision / PermissionPrompter ──
    ...
    fn matches(&self, tool_name: &str, input: &str) -> bool {
        ...
        match &self.matcher {
            ...
            PermissionRuleMatcher::Exact(expected) => {
                extract_permission_subject(input).is_some_and(|c| c == *expected)
            },
            PermissionRuleMatcher::Prefix(prefix) => {
                extract_permission_subject(input).is_some_and(|c| c.starts_with(prefix))
            },
        }
    }
}
...
impl PermissionPolicy {
    ...
    #[must_use]
    pub fn with_permission_rules_from_lists(
        mut self,
        allow: Vec<String>,
        deny: Vec<String>,
        ask: Vec<String>,
    ) -> Self {
        self.allow_rules = allow.iter().map(|r| PermissionRule::parse(r)).collect();
        self.deny_rules = deny.iter().map(|r| PermissionRule::parse(r)).collect();
        self.ask_rules = ask.iter().map(|r| PermissionRule::parse(r)).collect();
        self
    }
