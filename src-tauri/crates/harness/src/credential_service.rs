...
pub trait CredentialService: Send + Sync + std::fmt::Debug {
    ...
    async fn get_database_connection_string(&self, credential_id: &str) -> Result<String, String>;

    /// Resolve SMTP configuration from a credential.
    async fn get_smtp_config(&self, credential_id: &str) -> Result<SmtpServiceConfig, String>;

    /// Get authentication headers for a credential (suitable for HTTP requests).
    async fn get_auth_headers(&self, credential_id: &str) -> Result<Vec<(String, String)>, String>;
}
