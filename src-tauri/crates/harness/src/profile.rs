...
//! User Profile 契约
//!
//! 权威定义：`UserProfile` 及其附属类型（CodingStyleProfile、CommunicationProfile、
//! WorkHabitProfile、DomainKnowledgeProfile、LearningState、ProfileUpdate 等）
//! 统一在此处定义。trajectory 等下游 crate 通过 `pub use axagent_harness::profile::*`
//! 引用，不再重复定义（符合 AGENTS.md 铁律 4）。
...
/// 用户画像服务契约 — 下游实现层通过实现此 trait 提供具体存储能力
#[async_trait]
