...
pub fn validate_packet(packet: TaskPacket) -> Result<ValidatedPacket, TaskPacketValidationError> {
    ...
    if errors.is_empty() {
        ...
        fire_task_hook(axagent_runtime_core::hooks::HookEvent::TaskCreated, &packet);
...
/// 通知任务完成 — 触发 TaskCompleted hook (best-effort)
///
/// 由任务执行器在任务执行完毕后调用。
pub fn notify_task_completed(packet: &TaskPacket, success: bool, summary: &str) {
    fire_task_hook(axagent_runtime_core::hooks::HookEvent::TaskCompleted, packet);
    // 附加执行结果信息
    let _ = success;
    let _ = summary;
}
...
fn fire_task_hook(event: axagent_runtime_core::hooks::HookEvent, packet: &TaskPacket) {
    let runner = axagent_runtime_core::hooks::HookRunner::new(
        axagent_runtime_core::config::RuntimeHookConfig::default(),
    );
    let data = json!({
