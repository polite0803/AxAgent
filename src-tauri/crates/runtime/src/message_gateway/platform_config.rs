...
impl PlatformConfig {
    ...
    pub fn platforms(&self) -> Platforms {
        Platforms {
            slack: if self.slack_enabled {
                Some(crate::message_gateway::platforms::slack::SlackConfig {
                    bot_token: self.slack_bot_token.clone().unwrap_or_default(),
                    signing_secret: self.slack_signing_secret.clone().unwrap_or_default(),
                    workspace_id: self.slack_workspace_id.clone().unwrap_or_default(),
                })
            } else {
                None
            },
            whatsapp: if self.whatsapp_enabled {
                Some(crate::message_gateway::platforms::whatsapp::WhatsAppConfig {
                    phone_number_id: self.whatsapp_phone_number_id.clone().unwrap_or_default(),
                    access_token: self.whatsapp_access_token.clone().unwrap_or_default(),
                    business_account_id: self.whatsapp_business_account_id.clone().unwrap_or_default(),
                })
            } else {
                None
            },
...
pub struct Platforms {
    pub slack: Option<crate::message_gateway::platforms::slack::SlackConfig>,
    pub whatsapp: Option<crate::message_gateway::platforms::whatsapp::WhatsAppConfig>,
}
...
impl PlatformConfig {
    pub fn validate(&self) -> anyhow::Result<()> {
        ...
        if self.dingtalk_enabled && (self.dingtalk_app_key.is_none() || self.dingtalk_app_secret.is_none()) {
            anyhow::bail!(
                "Dingtalk app_key and app_secret are required when Dingtalk is enabled"
            );
        }
