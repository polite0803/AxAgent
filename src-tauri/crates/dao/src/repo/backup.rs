...
pub async fn restore_json_backup(
    db: &DatabaseConnection,
    backup_path: &str,
    strategy: &axagent_harness::types::RestoreStrategy,
) -> Result<axagent_harness::types::RestoreReport> {
    use axagent_harness::types::{RestoreReport, TableRestoreResult};
    ...
    if matches!(strategy, axagent_harness::types::RestoreStrategy::DryRun) {
        // 空跑模式：仅统计行数
