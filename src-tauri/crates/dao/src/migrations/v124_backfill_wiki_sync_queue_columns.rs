...
//! v124_backfill_wiki_sync_queue_columns: 补全 wiki_sync_queue 表缺失的
...
pub async fn up(db: sea_orm::DatabaseConnection) -> Result<(), DbErr> {
    ...
    for (table, column, col_type) in MISSING_COLUMNS {
        ...
        tracing::info!("[v124] 补列: {table}.{column} ({col_type})");
        added += 1;
    }

    tracing::info!(
        "[v124] 字段合规: {added} ADD, {already} 已存在 (共 {total})",
        added = added,
        already = already,
...
mod tests {
    ...
    #[tokio::test]
    async fn v124_is_self_idempotent() {
        let db = sea_orm::Database::connect("sqlite::memory:").await.unwrap();
        // 先跑 v100 建表
        super::super::v100_consolidated::up(db.clone()).await.unwrap();
        up(db.clone()).await.unwrap();
        // 第二次跑应不报错（幂等）
        up(db).await.expect("v124 必须可重复执行");
    }

    #[tokio::test]
    async fn v124_backfills_on_bare_table() {
        let db = sea_orm::Database::connect("sqlite::memory:").await.unwrap();
