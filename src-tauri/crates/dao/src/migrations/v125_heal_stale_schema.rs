...
//!
//! 由于版本表已 >= 123，`run_migrations` 永远不会重跑 v121。
//! 本迁移以 v125 单调递增，在版本滞后的存量库上必然执行，幂等补全。
//!
//! ## Strategy
//!
//! `agent_name`：先查缺（PG 用 information_schema，SQLite 用
//! pragma_table_info），再执行普通 `ADD COLUMN`（不用
//! `ADD COLUMN IF NOT EXISTS`——较老 SQLite 不支持该语法）。
//! 列类型 `TEXT`，可空，与 v121 完全一致。
//!
//! 全新库：v121 已建好 `agent_name`，本迁移为 no-op。
...
pub async fn up(db: sea_orm::DatabaseConnection) -> Result<(), DbErr> {
    ...
    if !exists {
        db.execute_unprepared("ALTER TABLE trajectory_trajectories ADD COLUMN agent_name TEXT")
            .await?;
        tracing::info!("[v125] trajectory_trajectories.agent_name 已补全");
    }

    tracing::info!("[v125] 存量库 schema 自愈完成");
    Ok(())
}
...
mod tests {
    ...
    /// 防回归：仅跑了 v100 的存量库（缺 agent_name），v125 必须补上该列。
    #[tokio::test]
    async fn v125_adds_agent_name_to_stale_db() {
        ...
        assert!(!column_exists(&db, "agent_name").await, "存量库在 v125 前应缺少 agent_name");

        up(db.clone()).await.expect("v125 应补全 agent_name");

        assert!(column_exists(&db, "agent_name").await, "v125 后 agent_name 应存在");
    }

    /// v125 单独幂等：重复跑不报错。
    #[tokio::test]
    async fn v125_is_self_idempotent() {
        let db = Database::connect("sqlite::memory:").await.expect("测试：连接数据库应成功");
        super::super::v100_consolidated::up(db.clone()).await.expect("测试：异步操作应成功");
        up(db.clone()).await.expect("测试：异步操作应成功");
        up(db).await.expect("v125 must be re-runnable in isolation");
    }
}
