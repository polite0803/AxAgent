...
## 禁区
- 不可提取仅与当前单次对话相关的信息（如"用户今天问了 X 问题"）
- 不可提取明显平庸的事实（如"用户使用的是计算机"）
- 不可编造知识——只提取对话中明确提到的内容
...
## 自验环节
输出前检查：是否有提取项是无用的琐碎信息？是否有提取项仅与当前对话相关？标题是否在 50 字符内？"#
        },
...
## 禁区
- 不可丢失原始记忆中的任何重要信息——检查合并后是否遗漏内容
- 不可添加原始记忆中不存在的新信息
- 不可改变原始记忆的事实含义

## 证据规则
- 合并后的标题和内容必须能从原始记忆中找到依据
- importance 评分必须使用原始记忆中的最高值

## 自验环节
输出前检查：合并后的记忆是否包含了所有原始记忆中的重要内容？是否有新增的不属于原始记忆的信息？"#
        },
...
- 不可提取对话中未明确提及的实体或关系
- 不可提取通用的、显而易见的实体
- 关系必须两端都有已提取的实体，不可出现孤立关系

## 证据规则
- 每个实体的描述必须能从对话中找到对应内容
- 每条关系的描述必须说明关系和对话中的依据

## 自验环节
输出前检查：所有实体是否在对话中有明确提及？所有关系是否连接了已提取的实体？是否遗漏了关键实体？"#
        },
...
## 自验环节
输出前检查：是否每项都是真正的新信息？是否避免了与已有知识的重复？是否遗漏了用户的关键决策或偏好变化？"#
        },
...
## 证据规则
- 每个子技能的功能边界必须从原始技能描述中找到依据
- 依赖关系必须说明依赖的数据或调用关系类型

## 自验环节
输出前检查：所有子技能是否都能独立开发和维护？依赖关系是否完整标注？是否有功能碎片化的问题？"#
        },
...
## 证据规则
- 每个子技能的接口定义必须说明与其他子技能的交互协议
- 依赖关系必须区分编译时依赖和运行时依赖

## 自验环节
输出前检查：是否有空壳子技能？接口描述是否清晰到其他人可以直接实现？依赖图是否为有向图（非循环依赖）？"#
        },
