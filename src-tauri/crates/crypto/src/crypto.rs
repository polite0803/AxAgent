...
#[cfg(test)]
static TEST_MACHINE_ID: parking_lot::Mutex<Option<String>> = parking_lot::Mutex::new(None);

fn get_machine_fingerprint() -> String {
    #[cfg(test)]
    {
        if let Some(id) = TEST_MACHINE_ID.lock().as_ref() {
            return id.clone();
        }
    }
...
mod tests {
    ...
    fn backup_key_encrypt_decrypt_roundtrip() {
        ...
        *super::TEST_MACHINE_ID.lock() = Some("0".repeat(64));
