...
// 各个语言的翻译（为简便起见，部分内容直接使用英文，或者参考相似翻译）
const translations = {
  'ar.json': {
    'chat.rewardWeights.title': 'أوزان المكافأة',
    'settings.theme': {
      label: 'السمة',
      title: 'مدير السمات',
  ...
  'de.json': {
    'chat.rewardWeights.title': 'Belohnungsgewichte',
    'settings.theme': {
      label: 'Thema',
  ...
  'es.json': {
    'chat.rewardWeights.title': 'Pesos de recompensa',
    'settings.theme': {
      label: 'Tema',
  ...
  'fr.json': {
    'chat.rewardWeights.title': 'Poids de récompense',
    'settings.theme': {
      label: 'Thème',
  ...
  'hi.json': {
    'chat.rewardWeights.title': 'पुरस्कार वेट्स',
    'settings.theme': {
      label: 'थीम',
      title: 'थीम प्रबंधक',
  ...
  'ja.json': {
    'chat.rewardWeights.title': '報酬ウェイト',
    'settings.theme': {
      label: 'テーマ',
      title: 'テーママネージャー',
  ...
  'ko.json': {
    'chat.rewardWeights.title': '보상 가중치',
    'settings.theme': {
      label: '테마',
      title: '테마 관리자',
  ...
  'ru.json': {
    'chat.rewardWeights.title': 'Веса вознаграждения',
    'settings.theme': {
      label: 'Тема',
      title: 'Менеджер тем',
...
for (const file of langs) {
  ...
  // 添加 chat.rewardWeights.title
  if (t['chat.rewardWeights.title'] && lang.chat?.rewardWeights && !('title' in lang.chat.rewardWeights)) {
    lang.chat.rewardWeights.title = t['chat.rewardWeights.title'];
    console.log(`${file}: Added chat.rewardWeights.title`);
  }

  // 添加 settings.theme
  if (t['settings.theme'] && lang.settings && !lang.settings.theme) {
    lang.settings.theme = t['settings.theme'];
    console.log(`${file}: Added settings.theme`);
  }

  // 添加 settings.shortcuts
  if (t['settings.shortcuts'] && lang.settings && !lang.settings.shortcuts) {
    lang.settings.shortcuts = t['settings.shortcuts'];
    console.log(`${file}: Added settings.shortcuts`);
  }

  // 添加 skills.marketplace
  if (t['skills.marketplace'] && lang.skills && !lang.skills.marketplace) {
    lang.skills.marketplace = t['skills.marketplace'];
    console.log(`${file}: Added skills.marketplace`);
  }

  // 添加 style.dimensions
  if (t['style.dimensions'] && lang.style && !lang.style.dimensions) {
