...
if [ "$MODE" = "strict" ]; then
  echo ""
  echo "--- Strict Mode: Additional Checks ---"

  # Check for message.format(...) with hardcoded strings
  > "$TEMP_DIR/r4.txt"
  for f in $CHANGED_FILES; do
    [ -f "$f" ] || continue
    grep -nP '(notification|message)\.(open|info|success|error|warning|loading)\(\s*[\x27\x22][^\x27\x22]{3,}[\x27\x22]' "$f" 2>/dev/null | while IFS=: read -r lnum content; do
      if ! is_allowed "$f" "$lnum"; then
        echo "  $f:$lnum: $content" >> "$TEMP_DIR/r4.txt"
      fi
    done || true
  done

  if [ -s "$TEMP_DIR/r4.txt" ]; then
    count=$(wc -l < "$TEMP_DIR/r4.txt")
    echo "  FAIL: $count notification/message hardcoded string(s):"
    cat "$TEMP_DIR/r4.txt"
    VIOLATIONS=$((VIOLATIONS + count))
    EXIT_CODE=1
  else
    echo "  PASS: No notification/message hardcoded strings"
  fi

  # title/label/content/description/tooltip/placeholder/text attributes with English-only strings
  > "$TEMP_DIR/r5.txt"
  for f in $CHANGED_FILES; do
    [ -f "$f" ] || continue
    grep -nP '\b(title|label|content|description|tooltip|placeholder|text)\s*=\s*"([A-Za-z][A-Za-z0-9\s\-_:/]+)"' "$f" 2>/dev/null | while IFS=: read -r lnum content; do
      if ! is_allowed "$f" "$lnum"; then
        echo "  $f:$lnum: $content" >> "$TEMP_DIR/r5.txt"
      fi
    done || true
  done

  if [ -s "$TEMP_DIR/r5.txt" ]; then
    count=$(wc -l < "$TEMP_DIR/r5.txt")
    echo "  FAIL: $count UI attribute(s) with hardcoded strings:"
    cat "$TEMP_DIR/r5.txt"
    VIOLATIONS=$((VIOLATIONS + count))
    EXIT_CODE=1
  else
    echo "  PASS: No UI attributes with hardcoded strings"
  fi
fi
