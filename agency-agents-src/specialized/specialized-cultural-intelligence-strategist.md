...
export function auditWorkflowForExclusion(uiComponent: UIComponent) {
  ...
  if (uiComponent.requires("firstName") && uiComponent.requires("lastName")) {
    auditReport.push({
      ...
      fix:
        'Combine into a single "Full Name" or "Preferred Name" field. Many global cultures do not use a strict First/Last dichotomy, use multiple surnames, or place the family name first.',
    });
  }
  ...
  if (uiComponent.theme.errorColor === "#FF0000" && uiComponent.targetMarket.includes("APAC")) {
    auditReport.push({
      ...
      fix:
        "In Chinese financial contexts, Red indicates positive growth. Ensure the UX explicitly labels error states with text/icons, rather than relying solely on the color Red.",
    });
  }
