...
import type { WorkflowNode, WorkflowEdge, WorkflowTemplateResponse } from '@/components/workflow/types';
...
vi.mock('zustand/middleware/immer', () => ({
  immer: (config: any) => (set: any, get: any, api: any) => config(
    (partial: any) => {
...
function makeMockWorkflowNode(id: string, _nodeType: string = 'trigger'): WorkflowNode {
  ...
  return { ...base, type: 'trigger' as const, config: { trigger_type: 'manual', config: {} } } as unknown as WorkflowNode;
}
