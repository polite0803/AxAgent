...
export const useConversationStore = create<ConversationState>((set, get) => ({
  ...
  setActiveConversation: (id) => {
    ...
    const _prefStore = _getPrefStore();
    if (_prefStore) {
  ...
  createConversation: async (title, model_id, providerId, options) => {
    try {
      ...
      const _prefStoreCreate = _getPrefStore();
      if (_prefStoreCreate) {
        _prefStoreCreate.setState(
  ...
  updateConversation: async (id, input) => {
    try {
      ...
      const _prefStoreUpdate = _getPrefStore();
      if (get().activeConversationId === id && _prefStoreUpdate) {
        _prefStoreUpdate.setState(
