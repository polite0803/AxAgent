import { Card, Timeline, Typography, Tag, Space, Tooltip, Empty, Badge } from 'antd';
import {
  SyncOutlined,
  LoadingOutlined,
  UploadOutlined,
...
export function OperationTimeline({ operations }: OperationTimelineProps) {
  ...
  if (operations.length === 0) {
    return (
      <Empty description={t('wiki.operation.noOperations')} image={Empty.PRESENTED_IMAGE_SIMPLE} />
    );
  }
  ...
  return (
    <Timeline
      ...
      items={operations.map((op) => ({
        dot: (
          <Badge status={getStatusColor(op.status) as 'success' | 'error' | 'processing' | 'default'} />
        ),
        children: (
