...
import { Button, Card, Space, Tag, Typography, theme, Tooltip, Modal } from 'antd';
import { Shield, ShieldCheck, ShieldX, ChevronDown, ChevronRight, Eye, Pencil, Terminal, AlertTriangle, Info, Clock } from 'lucide-react';
...
const RISK_CONFIG: Record<RiskLevel, { color: string; icon: React.ReactNode; label: string; tooltip: string; bgColor: string }> = {
  read_only: {
    color: 'blue',
    icon: <Eye size={14} />,
    label: 'Read Only',
...
function generateOperationSummary(toolName: string, input: Record<string, unknown>): string {
  ...
  if (lowerTool.includes('read') || lowerTool.includes('grep') || lowerTool.includes('glob') || lowerTool.includes('ls')) {
...
const PermissionCard: React.FC<PermissionCardProps> = ({
  ...
  const borderColor = status === 'pending'
    ? isDangerous ? token.colorErrorBorder
    : riskLevel === 'write' ? token.colorWarningBorder
    : token.colorWarningBorder
    : status === 'approved' ? token.colorSuccessBorder
    : status === 'denied' ? token.colorErrorBorder
    ...
    <>
        ...
        <Space orientation="vertical" style={{ width: '100%' }} size={12}>
          ...
          {status === 'pending' ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              ...
              <Tooltip title={t('common.learnAboutPermissions', 'Approving "Allow Once" grants permission for this single instance. "Always Allow" saves this decision for future use.')}>
            ...
            <Space>
              ...
              <Text type="warning" style={{ fontSize: 13 }}>{t('common.expired', 'Expired (Agent disconnected)')}</Text>
            </Space>
          )}
        </Space>
      </Card>
      ...
      >
        <Space direction="vertical" size={12}>
          <Text>
            {t('common.dangerousOperationWarning', 'This operation will execute commands that may have system-wide effects:')}
          </Text>
          <ul style={{ margin: 0, paddingLeft: 20 }}>
            <li><Text code>{toolName}</Text></li>
            {affectedPaths.slice(0, 3).map((p, i) => (
              <li key={i}><Text code style={{ fontSize: 12 }}>{p}</Text></li>
            ))}
          </ul>
          <Text type="secondary" style={{ fontSize: 12 }}>
