...
import { Badge, Card, Progress, Space, theme, Tooltip, Typography } from "antd";
import { Activity, Bot, CheckCircle2, ChevronDown, ChevronUp, Clock, Loader2, XCircle } from "lucide-react";
...
const AgentTaskList: React.FC<AgentTaskListProps> = ({ conversationId }) => {
  ...
  const statusText = agentStatus
    || (stats.running > 0 ? t("chat.agentTaskList.executing") : t("chat.agentTaskList.completed"));
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
            {isAgentActive && (
              <Tooltip title={t("chat.agentTaskList.agentRunning")}>
                <div
                  className="pulse-dot"
                  style={{ width: 8, height: 8, borderRadius: "50%", backgroundColor: token.colorPrimary }}
                />
              </Tooltip>
            )}
