      ...
      >
        ...
        <span
          className={`text-xs font-medium truncate ${
            selectedId === agent.id ? "text-blue-600 dark:text-blue-400" : ""
          }`}
        >
          {agent.name}
        </span>
...
const MultiAgentStatusPanel: React.FC = () => {
  ...
  const tabs = [
    {
      key: "agents" as const,
      label: `${t("chat.multiAgent.tabs.agents")} (${agents.length})`,
      icon: <Bot size={12} />,
    },
                ...
                {messages.length > 0
                  ? (
                    messages.map(msg => (
                      <div
                        key={msg.id}
                        className="flex items-center gap-2 px-3 py-1 text-xs border-b border-gray-100 dark:border-gray-800 last:border-b-0"
                      >
                        <span
                ...
                {memoryEntries.length > 0
                  ? (
                    memoryEntries.map((entry, i) => (
                      <div
                        key={i}
                        className="px-3 py-1 text-xs border-b border-gray-100 dark:border-gray-800 last:border-b-0"
                      >
                        <div className="flex items-center gap-2">
                          ...
                          {entry.owner_agent && (
                            <span className="text-gray-400">
                              {t("chat.multiAgent.by")} {entry.owner_agent.slice(0, 8)}
                            </span>
                          )}
                          {entry.ttl_secs && (
