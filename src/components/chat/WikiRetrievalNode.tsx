...
export function WikiRetrievalNode(
  ...
  if (status === "searching") {
    return (
      ...
      >
        ...
        >
          <Library size={16} style={{ color: token.colorPrimary }} />
        </span>
        <span style={{ color: token.colorTextSecondary, fontSize: 13 }}>
          {t("chat.wikiRetrieval.searching")}
        </span>
      </div>
    );
  }
  ...
  if (status === "error") {
    return (
      ...
      >
        <AlertCircle size={16} />
        <span>{node.content || t("chat.wikiRetrieval.error")}</span>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        <Library size={14} style={{ color: token.colorPrimary }} />
        <span style={{ fontSize: 13, fontWeight: 500 }}>
          {t("chat.wikiRetrieval.resultsCount", { count: totalItems })}
        </span>
        ...
        {sources.flatMap((src, si) =>
          src.items.map((item, ii) => (
            ...
            >
              ...
              <span
                style={{ color: token.colorPrimary, fontFamily: "var(--font-mono, 'JetBrains Mono', ui-monospace, monospace)" }}
              >
