...
import { Database, Layers, Zap, FileText } from 'lucide-react';
...
const ContextSummaryPreview: React.FC<ContextSummaryPreviewProps> = ({
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1 text-xs text-gray-500 dark:text-gray-400 select-none">
      {/* Message count */}
      <Tooltip title={`${summary.userMessages} user, ${summary.assistantMessages} assistant, ${summary.toolMessages} tool`}>
      ...
      {/* Context usage bar */}
      <Tooltip title={`${summary.usagePercent.toFixed(0)}% of context window used${summary.isNearLimit ? ' — auto-compression will trigger soon' : ''}`}>
        <div className="flex items-center gap-1">
          <Layers size={11} />
