  ...
  return (
    <div className="border-b border-border/50 px-3 py-2 space-y-2">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => { setExpanded(false); setFlushContent(''); }}
          ...
          {insights.slice(0, 8).map((insight) => (
            <div key={insight.id} className="text-xs p-1.5 rounded bg-muted/30">
              <div className="flex items-center gap-1 mb-0.5">
                <span className={`inline-block px-1 rounded text-[10px] font-medium ${CATEGORY_COLORS[insight.category]}`}>
      ...
      <div className="space-y-1">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <input
            ...
            onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleFlush(); } }}
