...
import { Card, Tag, Statistic, Row, Col, Button, Empty, theme } from 'antd';
import { PlayCircle, Power, RefreshCw, Clock, Zap } from 'lucide-react';
...
export function LinkOverview({ link }: LinkOverviewProps) {
  ...
  const statusColor = link.status === 'connected' ? 'green'
    : link.status === 'connecting' ? 'orange'
    : link.status === 'error' ? 'red'
    : 'default';

  const statusLabel = link.status === 'connected' ? t('link.statusConnected')
    : link.status === 'connecting' ? t('link.statusConnecting')
    : link.status === 'error' ? t('link.statusError')
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <Card size="small">
        <div className="flex items-center justify-between">
          ...
          >
            {isConnecting ? t('link.statusConnecting') : link.status === 'connected' ? t('link.disconnect') : t('link.connect')}
          </Button>
        </div>
      </Card>
