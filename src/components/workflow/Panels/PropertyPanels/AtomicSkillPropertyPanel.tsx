...
import { Input, Select, Button, Divider, Space, Tag } from 'antd';
...
export const AtomicSkillPropertyPanel: React.FC<AtomicSkillPropertyPanelProps> = ({
  node, onUpdate, onDelete, onEditSkill,
}) => {
      ...
      <div>
        ...
        <Space.Compact style={{ width: '100%' }}>
          <Select
            ...
            options={skills.map((s) => ({
              ...
              label: (
                <span>
                  {s.name}
                  <Tag style={{ marginLeft: 4, fontSize: 9 }} color={
                    s.entry_type === 'builtin' ? 'blue' :
                    s.entry_type === 'mcp' ? 'purple' :
                    s.entry_type === 'local' ? 'green' : 'orange'
                  }>
                    {s.entry_type}
                  </Tag>
                </span>
          ...
          {onEditSkill && config.skill_id && (
            <Button size="small" icon={<EditOutlined />} onClick={() => onEditSkill(config.skill_id)} />
          )}
        </Space.Compact>
        {selectedSkill && (
          ...
          {Object.entries(config.input_mapping || {}).map(([key, value]) => (
            <div key={key} style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
              ...
              <Input
                ...
                style={{ flex: 1 }}
              />
              <a style={{ fontSize: 10, color: '#ff4d4f' }} onClick={() => handleDeleteInputMapping(key)}>删除</a>
            </div>
          ))}
