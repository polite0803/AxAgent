 ...
 * - 可折叠（点击按钮或 Escape 键关闭）
 * - 左侧边缘可拖拽调整宽度（拖拽时禁用 transition 保证跟手）
...
export function AgentPanel() {
  ...
  const isDragging = useAgentPanelStore((s) => s.isDragging);
  const setWidth = useAgentPanelStore((s) => s.setWidth);
  const setDragging = useAgentPanelStore((s) => s.setDragging);
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      const handleMouseMove = (ev: MouseEvent) => {
        ...
        const newWidth = Math.min(PANEL_MAX_WIDTH, Math.max(PANEL_MIN_WIDTH, startWidthRef.current + delta));
        setWidth(newWidth);
      };
    ...
    [panelWidth, setWidth, setDragging],
  );
