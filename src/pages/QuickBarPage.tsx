...
type CommandType =
  | "chat" | "agent" | "url" | "search" | "wiki" | "calc" | "model"
  | "new" | "continue" | "memory" | "files" | "translate"
  | "summarize" | "code" | "settings" | "gateway";
...
const COMMAND_DEFS: (Omit<CommandDef, "labelKey" | "descKey"> & { labelKey: string; descKey: string })[] = [
  { key: "chat",      labelKey: "quickbar.command.chat",     descKey: "quickbar.command.chatDesc",     icon: <MessageSquare size={18} />,      category: "ai",        needsBody: true,  needsModel: true,  color: "#1677ff" },
  { key: "agent",     labelKey: "quickbar.command.agent",    descKey: "quickbar.command.agentDesc",    icon: <Zap size={18} />,                 category: "ai",        needsBody: true,  needsModel: true,  color: "#722ed1" },
  { key: "new",       labelKey: "quickbar.command.new",      descKey: "quickbar.command.newDesc",      icon: <MessageSquarePlus size={18} />,  category: "ai",        needsBody: false, needsModel: false, color: "#1677ff" },
  { key: "continue",  labelKey: "quickbar.command.continue", descKey: "quickbar.command.continueDesc", icon: <Blocks size={18} />,              category: "ai",        needsBody: false, needsModel: false, color: "#1677ff" },
  { key: "search",    labelKey: "quickbar.command.search",   descKey: "quickbar.command.searchDesc",   icon: <Search size={18} />,              category: "knowledge", needsBody: true,  needsModel: false, color: "#52c41a" },
  { key: "wiki",      labelKey: "quickbar.command.wiki",     descKey: "quickbar.command.wikiDesc",     icon: <BookOpen size={18} />,            category: "knowledge", needsBody: true,  needsModel: false, color: "#52c41a" },
  { key: "memory",    labelKey: "quickbar.command.memory",   descKey: "quickbar.command.memoryDesc",   icon: <MemoryStick size={18} />,          category: "knowledge", needsBody: true,  needsModel: false, color: "#52c41a" },
  { key: "files",     labelKey: "quickbar.command.files",    descKey: "quickbar.command.filesDesc",    icon: <FileSearch size={18} />,           category: "knowledge", needsBody: true,  needsModel: false, color: "#52c41a" },
  { key: "url",       labelKey: "quickbar.command.url",      descKey: "quickbar.command.urlDesc",      icon: <Globe size={18} />,               category: "web",       needsBody: true,  needsModel: true,  color: "#fa8c16" },
  { key: "summarize", labelKey: "quickbar.command.summarize",descKey: "quickbar.command.summarizeDesc",icon: <Replace size={18} />,             category: "web",       needsBody: true,  needsModel: true,  color: "#fa8c16" },
  { key: "translate", labelKey: "quickbar.command.translate",descKey: "quickbar.command.translateDesc",icon: <Languages size={18} />,            category: "web",       needsBody: true,  needsModel: true,  color: "#fa8c16" },
  { key: "calc",      labelKey: "quickbar.command.calc",     descKey: "quickbar.command.calcDesc",     icon: <Calculator size={18} />,           category: "tools",     needsBody: true,  needsModel: false, color: "#eb2f96" },
  { key: "code",      labelKey: "quickbar.command.code",     descKey: "quickbar.command.codeDesc",     icon: <Braces size={18} />,              category: "tools",     needsBody: true,  needsModel: false, color: "#eb2f96" },
  { key: "model",     labelKey: "quickbar.command.model",    descKey: "quickbar.command.modelDesc",    icon: <ChevronDown size={18} />,          category: "tools",     needsBody: false, needsModel: false, color: "#eb2f96" },
  { key: "settings",  labelKey: "quickbar.command.settings", descKey: "quickbar.command.settingsDesc", icon: <Settings size={18} />,             category: "system",    needsBody: false, needsModel: false, color: "#8c8c8c" },
  { key: "gateway",   labelKey: "quickbar.command.gateway",  descKey: "quickbar.command.gatewayDesc",  icon: <ArrowDownCircle size={18} />,      category: "system",    needsBody: false, needsModel: false, color: "#8c8c8c" },
];
...
function loadRecent(): string[] {
  try { const raw = localStorage.getItem(QUICKBAR_RECENT_KEY); return raw ? JSON.parse(raw) : []; } catch { return []; }
}

function saveRecent(items: string[]) {
  try { localStorage.setItem(QUICKBAR_RECENT_KEY, JSON.stringify(items.slice(0, 5))); } catch { /* noop */ }
}
...
export function QuickBarPage() {
  ...
  const currentProvider = useMemo(() => providers.find((p) => p.id === activeProviderId), [providers, activeProviderId]);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        ...
        if (commandMode) { setCommandMode(false); setInput(""); setActiveCommand(null); return; }
        handleHide();
      }
    };
  ...
  const startStream = useCallback(async (op: () => Promise<void>) => {
    ...
    try {
      ...
      const u1 = await listen<{ conversationId: string; assistantMessageId: string; text: string }>(
        "agent-stream-text", (event) => { text += event.payload.text; setResult(text); }
      );
  ...
  const runChat = (body: string) => startStream(async () => {
    const cid = await ensureConversation();
    await invoke("send_message", { conversationId: cid, content: body, providerId: activeProviderId, modelId: activeModelId });
  });

  const runAgent = (body: string) => startStream(async () => {
    const cid = await ensureConversation();
    await invoke("agent_query", { request: { conversationId: cid, input: body, providerId: activeProviderId, model_id: activeModelId } }, 0);
  });

  const runUrl = (url: string) => startStream(async () => {
    const cid = await ensureConversation();
    await invoke("agent_query", { request: { conversationId: cid, input: `Fetch the content from this URL and summarize it concisely: ${url}`, providerId: activeProviderId, model_id: activeModelId } }, 0);
  });

  const runSummarizeUrl = (url: string) => startStream(async () => {
    const cid = await ensureConversation();
    await invoke("agent_query", { request: { conversationId: cid, input: `Summarize the core content of this web page in 3-5 sentences: ${url}`, providerId: activeProviderId, model_id: activeModelId } }, 0);
  });

  const runTranslate = (text: string) => startStream(async () => {
    const cid = await ensureConversation();
    await invoke("agent_query", { request: { conversationId: cid, input: `Translate the following content to Chinese:\n\n${text}`, providerId: activeProviderId, model_id: activeModelId } }, 0);
  });
  ...
  const runSearch = async (body: string) => {
    ...
    try {
      ...
      if (!results || results.length === 0) { setResult(t("quickbar.result.noKnowledge")); setLoading(false); return; }
      setResult(results.map((r) => `**${r.title}** (${(r.score * 100).toFixed(0)}%)\n${r.content}`).join("\n\n---\n\n"));
    } catch (e) { setResult(`${t("quickbar.result.searchFailed")}: ${String(e)}`); }
    setLoading(false);
  };
  ...
  const runMemorySearch = async (body: string) => {
    ...
    try {
      const results = await invoke<Array<{ content: string; score: number; title: string }>>("search_knowledge_base", { query: body, limit: 5 });
      if (!results || results.length === 0) { setResult(t("quickbar.result.noMemory")); setLoading(false); return; }
      setResult(results.map((r) => `**${r.title}** (${(r.score * 100).toFixed(0)}%)\n${r.content}`).join("\n\n---\n\n"));
    } catch (e) { setResult(`${t("quickbar.result.searchFailed")}: ${String(e)}`); }
    setLoading(false);
  };
  ...
  const runCode = async (code: string) => startStream(async () => {
    const cid = await ensureConversation();
    await invoke("send_message", { conversationId: cid, content: `Execute the following code in a sandbox and return the result:\n\`\`\`\n${code}\n\`\`\``, providerId: activeProviderId, modelId: activeModelId });
  });
  ...
  const runNewConversation = useCallback(async () => {
    ...
    } catch (e) { setResult(`${t("quickbar.result.createFailed")}: ${String(e)}`); }
    setLoading(false);
  }, [t]);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (body) { pushRecent(input.trim()); setRecentItems(loadRecent()); }
    ...
    switch (command) {
      ...
      case "code": await runCode(body); break;
      case "memory": await runMemorySearch(body); break;
      case "translate": await runTranslate(body); break;
      case "summarize": await runSummarizeUrl(body); break;
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    if (showCommands) {
      ...
      if (e.key === "ArrowDown") { e.preventDefault(); setSelectedCmd((i) => Math.min(i + 1, visible.length - 1)); }
      else if (e.key === "ArrowUp") { e.preventDefault(); setSelectedCmd((i) => Math.max(i - 1, 0)); }
      else if (e.key === "Enter") {
        e.preventDefault();
        const cmd = visible[selectedCmd];
        if (cmd) { setInput(`/${cmd.key} `); setActiveCommand(cmd.key); setShowCommands(false); }
        setTimeout(() => inputRef.current?.focus(), 50);
      }
      return;
    }
  ...
  const clearAll = useCallback(() => {
    setInput(""); setResult(""); setActiveCommand(null); setCommandMode(false); setShowCommands(false);
  }, []);
  ...
  const actionBtnStyle = (color: string): React.CSSProperties => ({
    ...
    color, transition: "background-color 0.15s",
  });
        ...
        return (
          <div key={group.key} style={{ marginBottom: 16 }}>
            <div style={{
              ...
              <span style={{ width: 3, height: 12, borderRadius: 2, backgroundColor: group.borderColor, display: "inline-block" }} />
              {t(group.labelKey)}
            </div>
            <div style={{
              display: "grid",
  ...
  const renderCommandMode = () => (
    <>
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", flexShrink: 0, position: "relative" }}>
        <span
          onClick={() => { setCommandMode(false); setInput(""); setActiveCommand(null); }}
        ...
        {activeCmdDef && (
          <span style={{
            ...
            borderRadius: 4, flexShrink: 0, fontFamily: "var(--code-font-family, monospace)",
          }}>
            /{activeCmdDef.key}
          </span>
        )}
        ...
        {loading
          ...
          : showClear && (
            <button onClick={clearAll} style={{ background: "none", border: "none", cursor: "pointer", padding: 4, opacity: 0.4 }}>
      ...
      {showCommands && (
        <div style={{
          ...
          boxShadow: `0 4px 16px rgba(0,0,0,0.2)`, zIndex: 10, flexShrink: 0,
          ...
          {(() => {
            ...
            return visible.map((cmd, idx) => (
              <div
                key={cmd.key}
                onClick={() => { setInput(`/${cmd.key} `); setActiveCommand(cmd.key); setShowCommands(false); }}
                onMouseEnter={() => setSelectedCmd(idx)}
                style={{
                  display: "flex", alignItems: "center", gap: 10, padding: "6px 12px",
                  cursor: "pointer", fontSize: 12,
                  backgroundColor: idx === selectedCmd ? token.colorFillSecondary : "transparent",
                  color: token.colorText, transition: "background-color 0.1s",
                }}
              >
                <span style={{ color: cmd.color, display: "flex" }}>{cmd.icon}</span>
                <span style={{ fontWeight: 500, fontFamily: "var(--code-font-family, monospace)", minWidth: 50 }}>/{cmd.key}</span>
      ...
      {!hasResult && !showCommands && !showModelList && (
        <div style={{ display: "flex", gap: 10, padding: "2px 10px 6px", fontSize: 10, color: token.colorTextQuaternary, flexShrink: 0, overflow: "hidden", flexWrap: "wrap" }}>
          {COMMANDS.map((c) => (
            <span key={c.key} onClick={() => { setInput(`/${c.key} `); setActiveCommand(c.key); }}
      ...
      return (
        <>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "6px 0" }}>
            {currentModels.map((m) => (
              <div key={m.model_id} onClick={() => runModelSwitch(m.model_id)}
                style={{
                ...
                {m.model_id === activeModelId && <span style={{ fontSize: 10, color: token.colorPrimary }}>{t("quickbar.result.current")}</span>}
              </div>
            ))}
          </div>
        </>
    ...
    return (
      <>
        ...
        <div style={{
          ...
          maxHeight: "55vh", whiteSpace: "pre-wrap", wordBreak: "break-word",
        }}>
          <Typography.Text style={{ fontSize: 13 }}>{result}</Typography.Text>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 12px", borderTop: `1px solid ${borderColor}`, flexShrink: 0 }}>
  ...
  return (
    <div className="ax-page-transition" style={{
      height: "100vh", display: "flex", flexDirection: "column",
      backgroundColor: token.colorBgContainer, userSelect: "none",
    }}>
      {/* ── Header ──────────────────────────────────────────────────── */}
      <div className="ax-titlebar-compact title-bar-drag" style={{
        display: "flex", alignItems: "center", justifyContent: "space-between",
        paddingLeft: 14, paddingRight: 8, height: 28, flexShrink: 0,
        borderBottom: `1px solid ${borderColor}`,
      }}>
        <span style={{ fontSize: 12, fontWeight: 500, color: token.colorTextSecondary }}>
          {t("quickbar.title")}{activeCommand ? ` · /${activeCommand}` : ""}
        </span>
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          {activeModelId && (
            <Tooltip title={activeModelId}>
              <button className="title-bar-nodrag" onClick={() => setShowModelList((v) => !v)}
          ...
          <Tooltip title={commandMode ? t("quickbar.cmdTile") : t("quickbar.cmdLine")}>
            <button className="title-bar-nodrag" onClick={() => { setCommandMode(!commandMode); setInput(""); setActiveCommand(null); setShowCommands(false); }}
              style={{ background: "none", border: "none", cursor: "pointer", padding: 4, opacity: commandMode ? 0.8 : 0.4, color: token.colorTextSecondary }}>
              {commandMode ? <Slash size={14} /> : <Blocks size={14} />}
            </button>
          </Tooltip>
          <button className="title-bar-nodrag" onClick={handleHide}
            style={{ background: "none", border: "none", cursor: "pointer", padding: 4, opacity: 0.4, color: token.colorTextSecondary }}>
            <X size={14} />
          </button>
        </div>
      </div>
      ...
      {!commandMode ? (
        <>
          {recentItems.length > 0 && !hasResult && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 14px", fontSize: 11, color: token.colorTextQuaternary, flexShrink: 0, overflowX: "auto", whiteSpace: "nowrap" }}>
              <span style={{ flexShrink: 0 }}>{t("quickbar.recent")}:</span>
              {recentItems.map((item, i) => (
                <span key={i} onClick={() => { setCommandMode(true); setInput(item); setTimeout(() => inputRef.current?.focus(), 50); }}
                  style={{ cursor: "pointer", opacity: 0.65, maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis" }} title={item}>
                  {item.length > 30 ? item.slice(0, 30) + "…" : item}
                </span>
              ))}
              <span onClick={() => { saveRecent([]); setRecentItems([]); }}
                style={{ cursor: "pointer", opacity: 0.35, marginLeft: "auto", flexShrink: 0 }}>{t("quickbar.clearRecent")}</span>
            </div>
          )}
          {renderTileGrid()}
          {!hasResult && (
            <div style={{ padding: "6px 14px 10px", fontSize: 10, color: token.colorTextQuaternary, textAlign: "center", borderTop: `1px solid ${borderColor}`, flexShrink: 0 }}>
      ...
      {!commandMode && !hasResult && recentItems.length === 0 && (
        <div style={{ position: "absolute", bottom: 40, left: 0, right: 0, textAlign: "center", opacity: 0.08, pointerEvents: "none" }}>
